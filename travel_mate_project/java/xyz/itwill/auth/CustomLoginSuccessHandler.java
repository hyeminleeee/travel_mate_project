...
public class CustomLoginSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		//Authentication.getAuthorities() : 인증된 사용자에 대한 모든 권한 정보(GrantedAuthority 객체)
		//가 요소값으로 저장된 List 객체를 반환하는 메소드
		//GrantedAuthority 객체 : 사용자의 권한정보가 저장된 객체
		for(GrantedAuthority authority : authentication.getAuthorities()) {
			//GrantedAuthority.getAuthority() : GrantedAuthority 객체에 저장된 권한정보를 문자열로
			//반환하는 메소드
			roleNames.add(authority.getAuthority());
		}
		
		//Collection<T>.contains(T obj) : Collection 객체에 저장된 요소값을 매개변수로 전달받은 값과
		//비교하여 요소값이 없으면 [false]를 반환하고 요소값이 있으면 [true]를 반환하는 메소드
