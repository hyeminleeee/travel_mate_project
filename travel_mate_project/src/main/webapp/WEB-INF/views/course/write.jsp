...
<div class="container mt-5">
	<div class="main_div card p-4 shadow" >
			...
			<div class="option">
				<div>
					<form onsubmit="searchPlaces(); return false;">
						...
						<button type="submit" class="keyBtn" style="background:#cccccc;border:solid 1px #a1a1a1;border-radius:5px;">&nbsp;&nbsp;검색&nbsp;&nbsp;</button> 
		  			</form>
				</div>
			</div>
			<hr>
		...
		<%-- form 시작부 --%>    
		<form action="<c:url value="/course/write"/>" method="post" enctype="multipart/form-data" id="mainForm">
			...
			<%-- 제목, 대표사진 저장 --%>
			<input type="text" id="courseTitle" name="courseTitle" readonly="readonly" hidden="true">
			...
			<div class="btnDiv">
				...
				<button type="submit" class="imageFileBtn writeBtn">등록</button>
			</div>
		</form>
		<%-- form 종료부 --%>
...
function placesSearchCB(data, status, pagination) {
    if (status === kakao.maps.services.Status.OK) {
        ...
        // 제주도에 해당하는 장소만 필터링
        var jejuPlaces = data.filter(function(place) {
            return place.address_name.startsWith("제주특별자치도");
        });

        // 필터링된 결과가 없을 때 검색 목록을 비웁니다
        if (jejuPlaces.length === 0) {
            displayPlaces([]);  // 빈 배열을 넘겨서 검색 결과 목록을 비웁니다
            displayPagination({ last: 0, current: 0 }); // 페이지네이션도 초기화
            return;
        }

        // 필터링된 검색 목록과 마커를 표출합니다
        displayPlaces(jejuPlaces);
...
// 장소 추가
var eleno = 121; //게시글 번호 (이 후 변경 예정)
	    ...
	    "<div class='contentSetDiv-1' id='contentDiv" + index + "'>" +
			"<div class='contentSetDiv-1-1'>" +
				...
				"<div class='contentSetDiv-1-1-3'>" +
					"<button type='button' class='repButton imageFileBtn ChoiceBtn'>대표사진 지정</button>" +
				"</div>" +
			"</div>" +
...
$("#mainForm").submit(function (event) {
    ...
    // 대표 사진 검증
    if (!$('#mainImage')[0].files || $('#mainImage')[0].files.length === 0) {
...
$(document).on('change', '.inputImage', function() {
    ...
    if (file) {
        ...
        // img 태그의 src 속성에 이미지 URL을 설정하여 미리보기 표시
        /* $(this).nextAll('.imgView').first().attr('src', imageUrl); */
    }
});
